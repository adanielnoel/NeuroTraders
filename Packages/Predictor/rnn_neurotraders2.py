...
epochs1 = 400
batch_size1 = 64
learning_rate1 = 0.00025  # 0.001 is default for Adam optimizer -- usually an ok learning rate
train_percentage1 = 0.6  # Dataset must be split into three parts
train_percentage2 = 0.3
test_percentage2 = 0.1
n_features = 9  # n. of features flowing into first layer of the net
...
adam1 = optimizers.Adam(lr=learning_rate1, beta_1=0.9, beta_2=0.999, epsilon=1e-08,
                        decay=1e-2)  # this allows you to set the learning rate yourself
...
train2 = values_proc[
         int(train_percentage1 * len(values_proc)):int((train_percentage1 + train_percentage2) * len(values_proc)), :]
...
pickle.dump([yhat, yhat2, inv_yhat], open("predictions.pickle", "w"))
