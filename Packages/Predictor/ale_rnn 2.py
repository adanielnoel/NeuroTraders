...
from Packages.Predictor.Utils import Scaler, softmax
...
class RNNPredictor:
    def __init__(self, name, input_time_steps, input_length=1, output_length=1, lstm_size=128, num_layers=1):
        ...
        with self.graph.as_default():
            self.inputs_node = tf.placeholder(tf.float32, [None, self.input_time_steps, self.input_length],
                                              name="inputs")
            self.targets_node = tf.placeholder(tf.float32, [None, self.output_length], name="targets")
    ...
    def _loss_node(self):
        loss = tf.reduce_sum(tf.square(self.prediction_node - self.targets_node), name="loss_mse")
        baseline_loss = tf.reduce_sum(tf.square(self.targets_node), name="no_predict_loss")
    ...
    def train(self,
        ...
        self.input_scaler = Scaler(map_range=(-2.0, 2.0)).fit(train_X)
        self.output_scaler = Scaler(map_range=(-2.0, 2.0)).fit(train_Y)

        train_X = self.input_scaler.apply(train_X)
        train_Y = self.output_scaler.apply(train_Y)
        test_X = self.input_scaler.apply(test_X)
        test_Y = self.output_scaler.apply(test_Y)

        train_X = self.format_inputs(train_X)
        train_Y = self.format_targets(train_Y)
        test_X = self.format_inputs(test_X)
        test_Y = self.format_targets(test_Y)
        ...
        for epoch in range(max_epochs):
            ...
            for batch in range(train_X.shape[0] // batch_size):
                ...
                train_loss, _, summary = self.sess.run([self.loss_node,
                                                        self.optimize_node,
                                                        self.summary_node],
                                                       {self.inputs_node: batch_X,
                                                        self.targets_node: batch_Y,
            ...
            test_loss, baseline_los, summary = self.sess.run([self.loss_node, self.baseline_loss, self.summary_node],
                                                              {self.inputs_node: test_X,
                                                               self.targets_node: test_Y,
                                                               self.learning_rate_node: learning_rate,
                                                               self.keep_prob_node: keep_prob
                                                               })
            ...
            if batch % 10 == 0:
                self.writer.flush()
            print("Epoch:{:4}  lr: {:.6f} train_loss:{:.10f} test_loss:{:.4f}  baseline_loss:{:.4f}".format(
    ...
    #             series.append(self._format_inputs(data[:-i], format_for_training))
    ...
    #     if format_for_training:
    #         series = []
    #         for i in range(1, self.input_time_steps):
    #             series.append(self._format_targets(data[:-i], reshape, format_for_training))
    #         data = np.stack(series, axis=0)
    #     else:
    #         data = self._format_targets(data, reshape, format_for_training)
    #     return data
    ...
    def predict(self, input_x):
        if self.sess is None:
            self.launch()
        input_x = self.format_inputs(input_x, format_for_training=False)
        ...
        return self.output_scaler.revert(prediction)
...
if __name__ == "__main__":
    ...
    # selected_cols[:, 1] = np.sign(selected_cols[:, 1])
